...
For example, `ArrayList<Integer>` (Java), or `List[(String, Int)]` (Scala)

A <a style="color:#e7ad52">**dependent type**</a> is a type 
that is indexed by an *element* of another type. 

**Examples:**

1. The type of $n$-tuples is $A^n : \mathsf{Type}$  
has elements $(a_0, a_1, \dots, a_{n-1}) : A^n$ where $a_i : A$

2. The finite type $\underline{n} : \mathsf{Type}$  
has elements $0_n, 1_n, \dots, (n - 1)_n : \underline{n}$

---

Notice that tuples are indexed by both 
$\underline{n} : \mathsf{Type}$ *and* $A : \mathsf{Type}$, and we can
use a function into $\mathsf{Type} to represent such a 
<a style="color:#e7ad52">*dependent type*</a>

Vec : Type → $\mathbb N$ → Type
...
In the propositions as types view, **dependent types** are used to represent 
predicates, eg, $\mathsf{Prime} : \mathbb N \to \mathsf{Type}$ assigns to any 
natural number $n : \mathbb N$ the type of
evidence $\mathsf{Prime} n : \mathsf{Type}$ that $n$ is a prime number. 

(This does not need to be inhabited, eg $\mathsf{Prime} 4$ is empty.)

Using **currying** we can use this also to represent relations, eg, 
$\leq : \mathbb N \to \mathbb N \to \mathsf{Type}$, $m \leq n : \mathsf{Type}$ 
is the type of evidence that $m$ is less or equal to $n$.
